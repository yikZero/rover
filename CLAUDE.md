...
3. **Cache**: Static pages built at deploy time; dynamic routes (`/digests/[date]`, API endpoints, RSS) run as Node server endpoints with standard `Cache-Control` headers. Nginx serves `/_astro/` static assets directly.
...
Optional: `HOST` (default `0.0.0.0`), `PORT` (default `4321`)
